...
import sys
import pandas as pd
import Topspin as ts
##from Topspin import getval  
...
theta_deg=float(ts.Theta)
theta =theta_deg*pi/180
deltaT = float(ts.DeltaT)
...
def plot(userx,usery,theta,Cd,Cl,v_spin,rpm):
  ...
  e =float(ts.Res) #needs to be user input as well .75
  new_vx=vx
  new_vy=vy
...
# %%
userx = float(ts.x0)#26.83127295
usery = float(ts.y0)#7.18941792
